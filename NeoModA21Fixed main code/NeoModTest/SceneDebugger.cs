namespace NeoModTest
{
    ...
    public class SceneDebugger : MonoBehaviour
    {
        ...
        private void HierarchyWindowMethod(int id)
        {
            ...
            if (GUILayout.Button("Search", optionArray2))
            {
            }
            GUILayout.EndHorizontal();
            ...
            GUILayout.BeginHorizontal(new GUILayoutOption[0]);
            GUILayout.Label(this.SelectedGameObject.gameObject.layer + " : " + LayerMask.LayerToName(this.SelectedGameObject.gameObject.layer), new GUILayoutOption[0]);
            GUILayout.FlexibleSpace();
            GUILayoutOption[] optionArray4 = new GUILayoutOption[] { GUILayout.ExpandWidth(false) };
            this.SelectedGameObject.gameObject.SetActive(GUILayout.Toggle(this.SelectedGameObject.gameObject.activeSelf, "Active", optionArray4));
            ...
            {
                UnityEngine.Object.Destroy(this.SelectedGameObject.gameObject);
            }
            GUILayout.EndHorizontal();
        ...
        public void OnGUI()
        {
            this.HierarchyWindow = GUILayout.Window(this.HierarchyWindowId, this.HierarchyWindow, new GUI.WindowFunction(this.HierarchyWindowMethod), "Hierarchy", new GUILayoutOption[0]);
            this.ProjectWindow = GUILayout.Window(this.ProjectWindowId, this.ProjectWindow, new GUI.WindowFunction(this.ProjectWindowMethod), "Project", new GUILayoutOption[0]);
        }
        ...
        private void ProjectWindowMethod(int id)
        {
            ...
            foreach (Assembly assembly in assemblies)
            {
                ...
                {
                    List<System.Type> list = (from t in assembly.GetTypes()
                        where (t.IsClass && !t.IsAbstract) && !t.ContainsGenericParameters
                        select t).ToList<System.Type>();
                    foreach (System.Type type in list)
                    {
                        if (type.GetFields(BindingFlags.FlattenHierarchy | BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Static).Count<FieldInfo>(f => (f.Name != "OffsetOfInstanceIDInCPlusPlusObject")) != 0)
                        {
                            ...
                            {
                                FieldInfo[] fields = type.GetFields(BindingFlags.FlattenHierarchy | BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Static);
                                foreach (FieldInfo info in fields)
                                {
                                    if (info.Name != "OffsetOfInstanceIDInCPlusPlusObject")
                                    {
                                        ...
                                        GUILayout.Space(40f);
                                        GUILayoutOption[] optionArray4 = new GUILayoutOption[] { GUILayout.ExpandWidth(false) };
                                        this.ExpandedObjects[info] = GUILayout.Toggle(this.ExpandedObjects.ContainsKey(info) ? this.ExpandedObjects[info] : false, info.Name + " : " + info.FieldType, GUI.skin.label, optionArray4);
                                        GUILayout.EndHorizontal();
                                    }
                                }
                            }
        ...
        private sealed class <>c
        {
            ...
            public static Func<System.Type, bool> <>9__23_0;
            public static Func<FieldInfo, bool> <>9__23_1;

            internal bool <ProjectWindowMethod>b__23_0(System.Type t) => 
                ((t.IsClass && !t.IsAbstract) && !t.ContainsGenericParameters);

            internal bool <ProjectWindowMethod>b__23_1(FieldInfo f) => 
